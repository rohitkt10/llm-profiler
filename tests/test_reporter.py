...
def test_plot_memory_breakdown(mock_calc, mock_close, mock_save, tmp_path):
    ...
    mock_calc.return_value = (
        0.1  # Simplification: returns constant or we can side_effect
    )

    filepath = plot_memory_breakdown(
        sweep_results, str(tmp_path), "test/model", "none", model, tokenizer, 50, 4.0
    )
