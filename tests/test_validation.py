...
def test_validate_model_exists_invalid_hub_model(mock_model_info):
    ...
    with pytest.raises(
        click.BadParameter,
        match="Model 'nonexistent/model' not found on HuggingFace Hub.",
    ):
...
def test_validate_model_exists_generic_error(mock_model_info):
    ...
    with pytest.raises(
        click.BadParameter, match="Error checking model 'error/model': Network error"
    ):
...
def test_validate_compare_models_too_many_models(mock_model_info):
    ...
    with pytest.raises(
        click.BadParameter, match="Maximum 5 models allowed for comparison."
    ):
...
def test_validate_compare_models_with_invalid_model_in_list(mock_model_info):
    ...
    mock_model_info.side_effect = [
        MagicMock(),
        RepositoryNotFoundError("Invalid model"),
    ]
    with pytest.raises(
        click.BadParameter, match="Model 'invalid/model' not found on HuggingFace Hub."
    ):
