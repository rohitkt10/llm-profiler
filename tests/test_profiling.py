...
def mock_dependencies():
    with (
        patch("llm_profiler.cli.load_model") as mock_load,
        ...
        # Mock latency
        mock_latency.return_value = {10: 0.1, 25: 0.25, 50: 0.5, 100: 1.0, 200: 2.0}
...
def test_phase9_html_output(runner, mock_dependencies):
    ...
    result = runner.invoke(
        main, ["--model", "Qwen/Qwen2.5-0.5B-Instruct", "--output", "html"]
    )
...
def test_phase9_markdown_output(runner, mock_dependencies):
    ...
    result = runner.invoke(
        main, ["--model", "Qwen/Qwen2.5-0.5B-Instruct", "--output", "markdown"]
    )
