...
def measure_throughput(model, tokenizer, batch_size: int = 1, max_new_tokens: int = 50):
    ...
    with torch.no_grad():
        _ = model.generate(
            **inputs, max_new_tokens=5, pad_token_id=tokenizer.pad_token_id
        )
...
def sweep_batch_sizes(
    model, tokenizer, max_batch_size: int = 128, max_new_tokens: int = 50
):
    """
    ...
    for bs in batch_sizes:
        ...
        try:
            ...
            throughput, duration = measure_throughput(
                model, tokenizer, batch_size=bs, max_new_tokens=max_new_tokens
            )
            vram = get_vram_usage()
...
def find_oom_limit(results):
    ...
    """
    successful_bs = [
        bs for bs, res in results.items() if res.get("status") == "success"
    ]
...
def measure_prefill_decode(model, tokenizer, max_new_tokens: int = 50):
    ...
    # Ensure we don't exceed model context if model context is small
    ...
    decode_duration = max(
        0.0001, total_gen_duration - prefill_duration
    )  # Prevent div/0 or negative
    ...
    # Per token decode
    per_token_ms = (
        (decode_duration / max_new_tokens) * 1000 if max_new_tokens > 0 else 0.0
    )
...
def calculate_kv_cache_size(model, batch_size, seq_len):
    ...
    # Default to num_heads if not present (MHA)
    ...
    total_bytes = (
        2 * batch_size * seq_len * num_layers * num_kv_heads * head_dim * dtype_size
    )
...
def profile_memory_breakdown(model, tokenizer, batch_size=1, seq_len=100):
    ...
    if encoded.input_ids.shape[1] < seq_len:
        # In case seq_len is large, we might need padding or repeating
        pass
    ...
    kv_cache_mem = calculate_kv_cache_size(
        model, batch_size=batch_size, seq_len=seq_len + 1
    )

    # 4. Activations = Peak - Weights - KV Cache
    # This assumes Peak included Weights + Cache + Activations.
    # Sometimes peak happens during forward pass (activations max)
    # before KV cache is fully built?
    # Or KV cache grows.
...
def measure_output_length_impact(model, tokenizer):
    ...
    for length in lengths:
        try:
            # We use batch_size=1
            _, duration = measure_throughput(
                model, tokenizer, batch_size=1, max_new_tokens=length
            )
